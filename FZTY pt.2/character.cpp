...
{   
    //connect(Character::GetInstance(), SIGNAL(updateHappiness()), Character::GetInstance(), SLOT(updateChanges()));
    availableJobs =  {
        {"Нет", 0},
        {"Промоутер", 120},
        {"Курьер", 150},
    ...
    updateMonthlyIncome();
        ...
        timer->start(2 * 10 * 1000); // 4 seconds for an in-game day
...
void Character::setJob(const QString &job) {
   // if (monthsAtCurrentJob >= 3) {
        this->job = job;
        updateMonthlyIncome();
     //   monthsAtCurrentJob = 0; // Reset months at current job after job change
   // } else {
        // Optionally, you could throw an exception or handle this case as needed
  //  }
}
